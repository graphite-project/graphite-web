...
class StorageTest(TestCase):
    ...
    def test_store(self):
        ...
        old_remote_exclude_local = settings.REMOTE_STORE_EXCLUDE_LOCAL

        # Set test cluster servers
        settings.CLUSTER_SERVERS = ['127.0.0.1', '8.8.8.8']

        # Test REMOTE_STORE_EXCLUDE_LOCAL = False
        settings.REMOTE_STORE_EXCLUDE_LOCAL = False
        test_store = Store(settings.DATA_DIRS, remote_hosts=settings.CLUSTER_SERVERS)
        ...
        # Test REMOTE_STORE_EXCLUDE_LOCAL = True
        settings.REMOTE_STORE_EXCLUDE_LOCAL = True
        test_store = Store(settings.DATA_DIRS, remote_hosts=settings.CLUSTER_SERVERS)
        remote_hosts = [remote_store.host for remote_store in test_store.remote_stores]
        ...
        settings.REMOTE_STORE_EXCLUDE_LOCAL = old_remote_exclude_local
