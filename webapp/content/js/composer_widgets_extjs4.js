...
function createComposerWindow(myComposer) {
  ...
  var topToolbar = [
    ...
    {dock: 'top',xtype:"button",icon: STATIC_URL+"img/clock.png", tooltip: 'Select Recent Data', handler: toggleWindow(createRecentWindow)},
    
    {dock: 'top',xtype:"button",icon: STATIC_URL+"img/share.png", tooltip: 'Create from URL', handler: toggleWindow(createURLWindow)},
    {dock: 'top',xtype:"button",icon: STATIC_URL+"img/upload.png", tooltip: 'Short URL', handler: showShortUrl},
    timeDisplay
  ];
  ...
  var bottomToolbar = [
    {dock: 'bottom',xtype:"button",text: 'Graph Options', handler: createOptionsMenu},
    {dock: 'bottom',xtype:"button",text: 'Graph Data', handler: toggleWindow(Ext.bind(GraphDataWindow.create,GraphDataWindow))},
    {dock: 'bottom',xtype:"button",text: 'Auto-Refresh', enableToggle: true, toggleHandler: toggleAutoRefresh,id: 'autorefresh_button'},
  ];
...
function fitImageToWindow(win) {
  Composer.url.setParam('width', win.body.getWidth(true));
  Composer.url.setParam('height', win.body.getHeight(true));
  try {
...
function getCalendarSelection(which) {
  ...
  var myTime = Ext.getCmp(which + '-time').rawValue; // Need to grab the raw textfield value, which may not be selected
...
function saveMyGraph(button, e) {
  ...
  Ext.MessageBox.prompt(
	...
	return;
      ...
      Browser.trees.mygraphs.getStore().reload();
      //Send the request
      Ext.Ajax.request({
        method: 'GET',
...
var GraphDataWindow = {
  ...
  create: function () {
    ...
    this.targetList = new Ext.grid.Panel({
      store: TargetStore,
      multiSelect: true,
      ...
      width: 385,
      height: 140,
      columns: [ {text: "Graph Targets", flex: 1, dataIndex: "value"} ],
      listeners: {
        itemcontextmenu: this.targetContextMenu,
        afterrender: this.targetChanged,
        selectionchange: this.targetChanged,
        itemdblclick: function (targetList, index, node, e) {
      ...
      items: [
        {
          ...
          menu: {
            ...
            items: [
              { text: 'Move Up', handler: Ext.bind(this.moveTargetUp,this) },
              { text: 'Move Down', handler: Ext.bind(this.moveTargetDown,this) },
              { text: 'Swap', handler: Ext.bind(this.swapTargets,this), id: 'menuSwapTargets' }
            ]
          }
        }, {
          text: 'Apply Function',
    ...
    this.window = new Ext.Window({
      ...
      listeners: {
        afterrender: function () {
          if (_this.targetList.items.length > 0) {
            _this.targetList.getSelectionModel().select(0);
          }
        }
      }
    });
  ...
  targetChanged: function () {
    ...
    try {
      var selected =this.targetList.getSelectionModel().getSelection().length;
    } catch (e) {
      return;
    }
    if (selected == 0) {
  ...
  targetContextMenu: function (targetList, record, item, index, e) {
    ...
    var moveMenu = {
      ...
      menu: [
        { text: "Move Up", handler: Ext.bind(this.moveTargetUp, this) },
        { text: "Move Down", handler: Ext.bind(this.moveTargetDown, this) },
        { text: "Swap", handler: Ext.bind(this.swapTargets, this), disabled: true }
      ]
    };
    ...
    function applyFunc() {
      ...
      Ext.each(_this.getSelectedTargets(),
        function (target) {
          ...
          _this.targetList.getView().select( TargetStore.findExact('value', newTarget), true);
        }
      );
  ...
  applyFuncToAll: function (funcName) {
    function applyFunc() {
      ...
      Composer.syncTargetList();
      Composer.updateImage();
      this.targetList.getView().select( TargetStore.findExact('value', newTarget));
    }
    applyFunc = Ext.bind(applyFunc,this);
    return applyFunc;
  },
      ...
      function (target) {
        ...
        Ext.each(args, function (arg) {
          if (!arg.match(/^([0123456789\.]+|".+"|'.*')$/)) { //Skip string and number literals
            ...
            _this.targetList.getView().select( TargetStore.findExact('value', arg));
          }
        });
        Composer.syncTargetList();
        Composer.updateImage();
  ...
  editTarget: function (item, e) {
    var selected = this.targetList.getSelectionModel().getSelection();
  ...
  _moveTarget: function(direction) {
    ...
    selectedRecords = this.targetList.getSelectionModel().getSelection();
    ...
    Ext.each(selectedRecords, function(recordA) {
      ...
      newSelections.push( store.getAt(indexA + direction) );
    });
      
    Composer.syncTargetList();
    Composer.updateImage();
    this.targetList.getView().select(newSelections);
  },
  ...
  getSelectedTargets: function () {
    var targets = [];
    this.targetList.getSelectionModel().selected
    Ext.each(this.targetList.getSelectionModel().getSelection(), function (record) {
...
function createOptionsMenu(thisbutton,e) {
...
function createColorMenu(param) {
  var colorPicker = new Ext.picker.Color({hideOnClick: false});
