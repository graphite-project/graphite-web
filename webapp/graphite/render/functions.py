...
from graphite.render.attime import getUnitString, parseTimeOffset, parseATTime, SECONDS_STRING, MINUTES_STRING, \
    HOURS_STRING, DAYS_STRING, WEEKS_STRING, MONTHS_STRING, YEARS_STRING
    ...
    for series in seriesList:
        ...
        for i, value in enumerate(series):
            ...
            if value is None:
                ...
                if 0 < consecutiveNones <= limit:
                    for index in xrange(i - consecutiveNones, i):
                        series[index] = series[i - consecutiveNones - 1] + (index - (i - consecutiveNones - 1)) * (
                        value - series[i - consecutiveNones - 1]) / (consecutiveNones + 1)
...
def divideSeries(requestContext, dividendSeriesList, divisorSeries):
    ...
    if len(divisorSeries) > 1:
        raise ValueError(
            "divideSeries second argument must reference exactly 1 series (got {0})".format(len(divisorSeries)))
...
def weightedAverage(requestContext, seriesListAvg, seriesListWeight, *nodes):
    ...
    name = "weightedAverage(%s, %s, %s)" % (','.join(sorted(set(s.pathExpression for s in seriesListAvg))),
                                            ','.join(sorted(set(s.pathExpression for s in sorted(seriesListWeight)))),
                                            ','.join(map(str, nodes)))
    ...
    if isinstance(alignTo, bool):
        ...
        if alignTo is not None:
            ...
            if alignToUnit == YEARS_STRING:
                ...
                requestContext['startTime'] = datetime(s.year, s.month, s.day, tzinfo=s.tzinfo) - timedelta(
                    days=daysTosubtract)
                ...
                requestContext['startTime'] = datetime(s.year, s.month, s.day, s.hour, s.minute, s.second,
                                                       tzinfo=s.tzinfo)
...
def groupByTags(requestContext, seriesList, callback, *tags):
    ...
    for series in seriesList:
        ...
        if 'name' not in tags:
            ...
            key = ';'.join(
                [series.tags['name']] + sorted([tag + '=' + series.tags.get(tag, '') for tag in tags if tag != 'name']))
