...
def renderView(request):
  ...
  if requestOptions['graphType'] == 'pie':
    ...
    data = renderViewData(requestOptions['targets'], requestContext, useCache, cacheTimeout)
...
def renderViewData(targets, requestContext, useCache=False, cacheTimeout=None):
  # Let's see if at least our data is cached
  cachedData = None
  if useCache:
    startTime = requestContext['startTime']
    endTime = requestContext['endTime']
    dataKey = hashData(targets, startTime, endTime, requestContext['xFilesFactor'])
